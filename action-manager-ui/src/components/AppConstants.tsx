...
export const JOB_CATEGORY_VALUES: Array<SelectionData> = [
  { label: 'IO', value: 'IO' },
  { label: 'CPU', value: 'CPU' },
];
export const JOB_OUTPUT_TARGET_VALUES: Array<SelectionData> = [
...
export const isAllDependOnPropsValid = (
  dependOnArr: Array<any>,
  properties: Array<PropertyMetadata> | undefined
): boolean => {
  for (let index = 0; index < dependOnArr.length; index += 2) {
    const dependOnPropName = dependOnArr[index];
    const dependOnPropValue = dependOnArr[index + 1];
    const dependOnProp = findPropertyByCondition(
      properties,
      (p) => p.propName === dependOnPropName && p.propValue === dependOnPropValue
    );
...
export const findPropertyByCondition = (
  properties: Array<PropertyMetadata> | undefined,
  filter: (property: PropertyMetadata) => boolean
...
export const getActionDefinition = (properties: Array<PropertyMetadata>): ActionDefinition => {
  const actionDescription = findPropertyByCondition(
    properties,
    (property) => property.propName === 'actionDescription'
  );
  const actionConfigurations = findPropertyByCondition(
    properties,
    (property) => property.propName === 'actionConfigurations'
  );
  const actionStatus = findPropertyByCondition(
    properties,
    (property) => property.propName === 'actionStatus'
  );
  const name = findPropertyByCondition(
    properties,
    (property) => property.propName === 'actionName'
  );
...
export const getJobDefinition = (properties: Array<PropertyMetadata>) => {
  const name = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('name')
  )?.propValue;
  const description = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('description')
  )?.propValue;
  const configurations = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('configurations')
  )?.propValue;
  const content = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('content')
  )?.propValue;
  const isAsync = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('isAsync')
  )?.propValue;
  const category = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('category')
  )?.propValue;
  const outputTargets = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('outputTargets')
  )?.propValue;
  const isScheduled = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('isScheduled')
  )?.propValue;
  const scheduleInterval = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('scheduleInterval')
  )?.propValue;
  const status = findPropertyByCondition(properties, (property) =>
    property.propName.startsWith('status')
  )?.propValue;
  const templateArrays = findPropertyByCondition(properties, (property) =>
...
export const getJobDetails = (currentStepMetadata: Array<StepMetadata>) => {
  const findRelatedJobs = (currentStepMetadata: Array<StepMetadata>): Array<JobDefinition> => {
    return currentStepMetadata.map((stepMetadata) => getJobDefinition(stepMetadata.properties));
  };
...
export class ActionAPI {
  static setFavoriteAction = async (
    actionId: string,
    isFavorite: boolean,
  ...
  static loadTrashSummarysAsync = async (
    ...
    await restClient.sendRequest(requestOptions, targetURL, async (response) => {
      const actionPagingResult = (await response.json()) as PagingResult;
  ...
  static loadActionDetailAsync = async (
    actionId: string,
    restClient: RestClient,
    successCallback: (response: ActionDetails) => void
  ) => {
    const requestOptions = {
      method: 'GET',
      headers: {
    ...
    await restClient.sendRequest(requestOptions, targetURL, async (response) => {
      const actionDetailResult = await response.json();
  ...
  static updateAction = async (
    ...
    await restClient.sendRequest(requestOptions, targetURL, async (response) => {
      successCallback();
      return {
        message: 'Action info has been updated successfully',
  ...
  static export = async (actionId: string, actionName: string, restClient: RestClient) => {
    ...
    await restClient.sendRequest(requestOptions, targetURL, async (response) => {
      ...
      const fileName = response.headers.get('X-DOWNLOAD-FILE-NAME');
      fileDownload(blob, fileName || `${actionName}.zip`);
      return {
        message: 'The action is export sucessfully',
        key: new Date().getTime(),
      } as SnackbarMessage;
    });
  };

  static archive = async (
    actionId: string,
    restClient: RestClient,
    successCallback: () => void
  ) => {
...
export class TemplateAPI {
  static async search(
    name: string,
    restClient: RestClient,
    successCallback: (templateOverviews: Array<TemplateOverview>) => void
  ) {
    const requestOptions = {
      method: 'GET',
      headers: {},
...
export class JobAPI {
  static update = async (
    ...
    await restClient.sendRequest(requestOptions, targetURL, async (response) => {
      successCallback();
      return {
        message: 'Job info has been updated successfully',
        key: new Date().getTime(),
      } as SnackbarMessage;
    });
  };
  ...
  static async load(jobId: string, restClient: RestClient, successCallback: (data: any) => void) {
    ...
    const targetURL = `${JOB_MANAGER_API_URL}/${jobId}`;
    await restClient.sendRequest(requestOptions, targetURL, async (response) => {
      const jobDetail: JobDetailMetadata = (await response.json()) as JobDetailMetadata;
