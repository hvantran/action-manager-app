...
export default function ActionCreation() {
  ...
  const initialStepsV3: Array<StepMetadata> = [];
  ...
  const [actionPropertyMetadata, setActionPropertyMetadata] = React.useState<
      ...
      },
      {
        ...
        autoCompleteMeta: {
          ...
          onChange: function (event: React.SyntheticEvent, value: Array<TemplateOverview>, reason) {
            ...
            setStepMetadatas(
              onchangeStepDefault(
                `configurations${jobIndex}`,
                JSON.stringify(configurations, undefined, 2)
              )
            );
          ...
          onSearchTextChangeEvent: function (event: any) {
            const propValue = event.target.value;
            const propName = event.target.name;
            const jobIndex = propName.replace('templates', '');
            TemplateAPI.search(propValue, restClient, (templateOverviews) => {
              setStepMetadatas(
                onchangeStepDefault(propName, [], undefined, (property: PropertyMetadata) => {
  ...
  const initialStepMetadatas: Array<StepMetadata> = [
    ...
    },
    {
      ...
      onFinishStepClick: async (currentStepMetadata: Array<StepMetadata>) => {
        const action: ActionDefinition = getActionFromStepper(currentStepMetadata);
        ...
        await restClient.sendRequest(requestOptions, targetURL, async (response) => {
          ...
          const message = `Action ${responseJSON['actionId']} is created`;
  ...
  const dryRunAction = async () => {
    const action: ActionDefinition = getActionFromStepper(stepMetadatas);
  ...
  const getActionFromStepper = (currentStepMetadata: Array<StepMetadata>) => {
    const actionMetadata = currentStepMetadata.at(0);
    ...
    const actionDefinition = getActionDefinition(actionMetadata.properties);
    actionDefinition.jobs = findRelatedJobs(currentStepMetadata);
    return actionDefinition;
  };

  const initialFloatingActions: Array<SpeedDialActionMetadata> = [
    {
      ...
      onClick: () => {
        setStepMetadatas((previous) => {
          ...
          const nextIndex = nextStepMetadata.length;
          const newStepName = `${initialTemplateStep.name}${nextIndex}`;
          ...
          const templatePropertyOptions = findPropertyByCondition(
            propertyMetadata,
            (p) => p.propName === 'templates'
          )?.autoCompleteMeta?.options;
          const newTemplateProperty = findPropertyByCondition(
            addtionalStep.properties,
            (p) => p.propName === `templates${nextStepMetadata.length}`
          )?.autoCompleteMeta;
    ...
    {
      actionIcon: WithLink('/actions', <ArrowBackTwoToneIcon />),
      actionName: 'navigateBackToActions',
      actionLabel: 'Navigate to actions',
      properties: {
        sx: {
          bgcolor: blue[500],
          '&:hover': {
