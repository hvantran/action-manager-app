...
export function onchangeStepDefault(
  propName: string,
  propValue: any,
  stepMetadataCallback?: (stepMetadata: StepMetadata) => void,
  propertyCallback?: (property: PropertyMetadata) => void
): React.SetStateAction<StepMetadata[]> {
  return (previous) => {
    return [...previous].map((stepMetadata) => {
      const properties = stepMetadata.properties.map((prop) => {
...
export function onChangeProperty(
  propName: string,
  propValue: any,
  propertyCallback?: (property: PropertyMetadata) => void,
  valueCallback?: (property: PropertyMetadata, value: any) => void
): React.SetStateAction<PropertyMetadata[]> {
  return (previous) => {
...
export class RestClient {
  ...
  async sendRequest(
    ...
    const responsePromise = this.getFetchRequest(
      requestOptions,
      targetURL,
      successCallback,
      errorCallback
  ...
  async getFetchRequest(
    ...
    try {
      ...
      if (response.status >= 400) {
        ...
        const errorSnackbarMessage = errorCallback(response);
        if (errorSnackbarMessage) {
          const snackbarMessage = await errorSnackbarMessage;
          if (snackbarMessage) {
      ...
      const successSnackbarMessage = successCallback(response);
      if (successSnackbarMessage) {
        const snackbarMessage = await successSnackbarMessage;
        if (snackbarMessage) {
