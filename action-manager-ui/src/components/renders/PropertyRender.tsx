...
export default function PropertyRender(props: any) {
  const property: PropertyMetadata = props.property;
  const value =
    !property.propValue || property.propValue === ''
      ? property.propDefaultValue
        ? property.propDefaultValue
        : ''
      : property.propValue;
  ...
  switch (property.propType) {
    case PropType.CodeEditor:
      ...
      const codeEditorMeta = property.codeEditorMeta;
      renderNode = (
        <CodeEditor
    ...
    case PropType.Textarea:
      ...
      const textareaFieldMeta = property.textareaFieldMeta;
      renderNode = (
        <TextField
          key={property.propName}
    ...
    case PropType.Switcher:
      ...
      const switcherFieldMeta = property.switcherFieldMeta;
      renderNode = (
        <Switch
          key={property.propName}
          name={property.propName}
    ...
    case PropType.Selection:
      ...
      const selectionMeta = property.selectionMeta;
      renderNode = (
        <FormControl sx={{ m: 1, width: '100%', margin: 0 }} size="small">
          ...
          >
            ...
            {selectionMeta.selections.map((selection) => {
              return (
                <MenuItem key={property.propName + selection.value} value={selection.value}>
                  {selection.label}
                </MenuItem>
              );
            })}
    ...
    case PropType.Autocomplete:
      ...
      const autoCompleteMeta = property.autoCompleteMeta;
      renderNode = (
        <Autocomplete
          key={property.propName}
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              name={property.propName}
              onChange={autoCompleteMeta.onSearchTextChangeEvent}
              placeholder="..."
            />
          )}
          sx={{ width: '100%' }}
    ...
    default:
      ...
      const textFieldMeta = property.textFieldMeta;
      renderNode = (
        <Input
          required={property.isRequired}
          size="small"
      ...
      <Grid container spacing={2}>
        <Grid item {...property.labelElementProperties}>
          <Box {...property.labelElementProperties.sx}>
            <StyledLabel>
              {property.propLabel} {property.isRequired ? <span>*</span> : <span />}
            </StyledLabel>
            {property.info && (
