...
export default function TableRender(props: TableMetadata) {
  ...
  const handleChangeOrder = (orderByColumn: string) => (event: React.MouseEvent<unknown>) => {
    ...
    const previousColumnName = orderBy.replace('-', '');
  ...
  const recordTransfromCallback = props.pagingResult.elementTransformCallback;
                  ...
                  {props.columns.map((column) => {
                    ...
                    return (
                      <StyledTableCell
                        onMouseDown={(event) => {
                          event.button === 1 &&
                            props.onMouseWheelClick &&
                            props.onMouseWheelClick(row);
                        }}
